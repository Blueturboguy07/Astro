...
export async function testProvider(
  ...
  try {
    ...
    // round-trip to the local Astro server that hosts
    ...
    // as "Astro dropped the port I typed" (see issue #1844).
    ...
    return {
      success: false,
      message:
        `Could not reach the local Astro server at ${agentServerUrl}. ` +
        `Make sure Astro is running and try again. (${detail})`,
      responseTime,
    }
  }
}
