/* publik API — the metered provider the packaged Astro ships with.
 *
 ...
 * purpose: global fetch, @wxt-dev/storage and the two provider stores the
 * app already has are all it touches. */

export const PUBLIK_APP_SLUG = 'astro'
...
/* Picker order: balanced first so it is the default. */
export const PUBLIK_TIER_ORDER: PublikTier[] = ['balanced', 'fast', 'smart']

/* The two provider stores the key lands in (CONTRACT §0: "its existing
   credential store"). The answer engine reads `~/.browseros/simplicity/
   config.json` through the local server's /api/providers; the browser agent
   reads chrome.storage.local['llm-providers']. Both entries carry this id
   or name so every surface can find them again. */
export const PUBLIK_PROVIDER_NAME = 'publik API'
export const PUBLIK_AGENT_PROVIDER_ID = 'publik-api'
...
export type PublikState = {
  ...
  /* pending:      token present; not yet minted (disclosure not accepted,
     ...
     active:       the key is in both provider stores
  ...
  /* The answer-engine provider id the local server assigned. */
  ...
  weekResetsAt?: string
  walletSeenAt?: number
  /* The last 402 the chat surfaced, cleared by the next wallet read that
     shows usage available. */
  creditError?: { message: string; topUpUrl: string | null }
  mintedAt?: string
...
/* What the UI reads: state and money, never the token or the key. */
export type PublikStatus = {
  /* False without a build-time app token (dev, source builds): every
     publik surface hides itself. */
  available: boolean
