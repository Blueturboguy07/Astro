...
class OpenAILLM extends BaseLLM<OpenAIConfig> {
  ...
  constructor(protected override config: OpenAIConfig) {
    super(config);
        ...
        toolCalls:
          response.choices[0].message.tool_calls
            ?.map((tc) => {
              ...
              /* Non-function tool calls are dropped by the filter below; the
                 explicit return satisfies noImplicitReturns. */
              return undefined;
            })
