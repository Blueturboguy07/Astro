...
import NullEmbedding from './base/null-embedding';
...
class ModelRegistry {
  ...
  async loadEmbeddingModel(providerId: string, modelName: string) {
    ...
    /* No embedding provider configured. See ./base/null-embedding for why this
       degrades instead of throwing — the client sends an empty providerId when
       no provider exposes embedding models, which is the normal case on
       BrowserOS's default Claude Code provider. */
    if (!providerId || !provider) return new NullEmbedding();
