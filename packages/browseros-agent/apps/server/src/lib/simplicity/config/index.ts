...
/* Upstream wrote ./data/config.json relative to process.cwd(). The server's cwd
   is not a writable app-state location (and under `bun test` it is the package
   root, where ./data does not exist), so this follows the BrowserOS data dir the
   way the rest of the app's state does. */
...
class ConfigManager {
  configPath: string = path.join(resolveConfigDir(), 'config.json');
  configVersion = 1;
...
/* Constructed on first property access rather than at module scope. Upstream
   instantiated eagerly, which meant importing any route that touches config
   created directories and wrote a file — enough to fail boot tests that assert
   where state lands. */
let instance: ConfigManager | undefined;

const configManager = new Proxy({} as ConfigManager, {
